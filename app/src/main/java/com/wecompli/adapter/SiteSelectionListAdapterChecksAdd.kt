...
class SiteSelectionListAdapterChecksAdd(
    val activity: MainActivity,
    val siteList: ArrayList<CategorySiteListResponse.Row>,
    val AddChecks: AddChecksFragment
)
    : RecyclerView.Adapter<SiteSelectionListAdapterChecksAdd.ViewHolder>(){
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        if ( AddChecks.siteListRow!!.get(position).isselect) {
        ...
        itemView!!.chkSelect.setOnCheckedChangeListener { compoundButton, b ->
            if (b){
                AddChecks.siteListRow!!.get(position).isselect=true
            }else
                AddChecks.siteListRow!!.get(position).isselect=false
        }
